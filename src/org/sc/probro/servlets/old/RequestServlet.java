...
public class RequestServlet extends SkeletonDBServlet {
		...
		try { 
			...
			try { 
				...
				try { 
					...
					ProvisionalTermObject provisionalTerm = model.getProvisionalTerm(provisional_id);
					Log.info(String.format("found provisionalterms.provisional_id=%d", provisionalTerm.provisional_id));

					RequestObject loaded = model.getLatestRequest(provisionalTerm);
					Log.info(String.format("found requests.request_id=%d", loaded.request_id));
					
					Collection<MetadataObject> metadata = model.getMetadata(loaded);
					...
					if(responseType.equals("application/json")) {
						...
						writer.key("metadata");
						writer.array();
						for(MetadataObject md : metadata) { 
						...
						MetadataObject mdheader = new MetadataObject();
	...
	public RequestObject readRequestFromParams(Map<String,String[]> params) { 
		RequestObject req = new RequestObject();
		req.setFromParameters(params);
		return req;
	}
	
	public RequestObject readRequestFromJSON(JSONObject obj) { 
		RequestObject req = new RequestObject();
		req.setFromJSON(obj);
		return req;
	}
		...
		try { 
			...
			try {
				...
				try { 
					...
					ProvisionalTermObject provisionalTerm = model.getProvisionalTerm(provisional_id);
					RequestObject dbReq = model.getLatestRequest(provisionalTerm);
					...
					RequestObject newReq = new RequestObject();
